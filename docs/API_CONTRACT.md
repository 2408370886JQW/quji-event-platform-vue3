...
| PUT  | `/organizers/:id/onboarding/identity`       | 姓名、身份证号、经办人身份、授权确认           | 最新 `OnboardingState`      |
...
建议服务端枚举：`not_started`、`identity_completed`、`materials_draft`、`submitted`、`changes_required`、`approved`。材料应包含 `key`、`required`、`status`、`fileId`、`fileName`、可控时效的预览 URL、审核意见、版本与更新时间；原件下载必须独立授权。
