...
主办方只需上传一次营业执照，前端适配层会自动读取并关联主体名称、统一社会信用代码、法定代表人、成立日期、营业期限和登记住所；接入真实后端时替换为 `src/api/onboarding.ts` 中的 OCR 接口。桌面端将法定代表人身份证材料固定在左栏、营业执照固定在同一行右栏；身份证正反面合并为左栏同一张材料卡内的两个上传位。窄屏下两栏按身份证、营业执照顺序纵向排列。选择“被授权经办人办理”时，系统会额外显示经办授权书并纳入必填校验；选择“法定代表人本人办理”时不要求授权书。经营性业务许可按活动性质选填。
